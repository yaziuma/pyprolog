...
class MathInterpreter(Visitor):
    ...
    def _compute_binary_operand(self, left, operand, right):
        ...
        if operand == "*":
            ...
            if not isinstance(left, prolog_types.Number) or not isinstance(
                right, prolog_types.Number
            ):
                raise InterpreterError(
                    f"Operands for 'mod' must be Numbers. Got {type(left)} and {type(right)}"
                )
            if right.pred == 0:
            ...
            operand == "//" or operand == "div"
        ):  # Supporting both // and div for integer division
            if not isinstance(left, prolog_types.Number) or not isinstance(
                right, prolog_types.Number
            ):
                raise InterpreterError(
                    f"Operands for '//' or 'div' must be Numbers. Got {type(left)} and {type(right)}"
                )
            if right.pred == 0:
