...
class Variable:
    ...
    def match(self, other, bindings=None): # bindings param for compatibility, not used by new unify
        ...
        if self.name == '_': # Anonymous variable matches anything
            ...
            return bindings
        if isinstance(other, Variable) and other.name == '_': # Matching against anonymous var
    ...
    def substitute(self, bindings):
        ...
        for arg in self.args:
            if hasattr(arg, 'substitute'):
                ...
                substituted_args.append(arg) # Arg is not substitutable (e.g. Python constant)
...
class Rule: # Basic Rule structure
    def __init__(self, head, body):
        if not isinstance(head, Term):
            ...
            pass
        if not (isinstance(body, Term) or isinstance(body, Conjunction) or \
                body is TRUE() or body is FALSE() or isinstance(body, Variable)): # body can be complex
    ...
    def substitute(self, bindings):
        ...
        new_head = self.head.substitute(bindings) if hasattr(self.head, 'substitute') else self.head
        new_body = self.body.substitute(bindings) if hasattr(self.body, 'substitute') else self.body
...
class Conjunction(Term): # Conjunction is a special kind of Term
    def __init__(self, goals): # goals is a list of terms/conj/etc.
        super().__init__(',', *goals) # Use ',' as predicate for conjunction, args are the goals

    def substitute(self, bindings):
        # logger.debug(f"Conjunction.substitute({self}) called with bindings: {bindings}")
        substituted_goals = [g.substitute(bindings) if hasattr(g, 'substitute') else g for g in self.args]
