...
class TestListOperations:
    ...
    def test_member_generation_mode(self):
        ...
        self.assertQuerySolutions("member(X, [a,b,c])", [
            {"X": Atom("a")},
            {"X": Atom("b")},
            {"X": Atom("c")}
        ])
        self.assertQuerySolutions("member(X, [a])", [{"X": Atom("a")}])
        self.assertQuerySolutions("member(X, [a,a,b])", [ # Check for duplicate elements
            {"X": Atom("a")}, # First 'a'
            {"X": Atom("a")}, # Second 'a'
            {"X": Atom("b")}
        ])
    ...
    def test_member_element_and_list_vars(self):
        ...
        self.assertQuerySolutions("member(X, [a,b])", [{"X":Atom("a")},{"X":Atom("b")}]) # Same as generation
        ...
        solutions = self.runtime.query("member(a, [X,b])") # X will be 'a' for the first solution
        ...
        self.assertQuerySolutions("member(a, [X,Y])", [
        ...
        self.assertQuerySolutions("member(Val, [foo,bar])", [{"Val": Atom("foo")}, {"Val": Atom("bar")}])
    ...
    def test_member_complex_elements(self):
        ...
        self.assertQuerySolutions("member(X, [f(a), g(b)])", [
            {"X": Term(Atom("f"), [Atom("a")])},
            {"X": Term(Atom("g"), [Atom("b")])}
        ])
        ...
        self.assertQuerySolutions("member(item(foo), [item(Val)])", [{"Val": Atom("foo")}])
    ...
    def test_append_predicate(self):
        ...
        self.assertQueryTrue("append([a,b], [c,d], L3)", [{"L3": make_list(Atom("a"), Atom("b"), Atom("c"), Atom("d"))}])
        self.assertQueryTrue("append([], [a,b], L3)", [{"L3": make_list(Atom("a"), Atom("b"))}])
        self.assertQueryTrue("append([a,b], [], L3)", [{"L3": make_list(Atom("a"), Atom("b"))}])
        ...
        # L1=[a,b], L2=[c]
        # L1=[a,b,c], L2=[]
        self.assertQuerySolutions("append(L1, L2, [a,b,c])", [
        ...
        self.assertQuerySolutions("append(L1, L2, [a])", [
            {"L1": Atom("[]"), "L2": make_list(Atom("a"))},
            {"L1": make_list(Atom("a")), "L2": Atom("[]")}
        ])

        self.assertQuerySolutions("append(L1, L2, [])", [
            {"L1": Atom("[]"), "L2": Atom("[]")}
        ])

        # 4. Other modes (mixed variables and ground terms)
        self.assertQueryTrue("append([a,X], [c,Y], [a,b,c,d])", [{"X": Atom("b"), "Y": Atom("d")}])
        self.assertQueryTrue("append(L1, [c,d], [a,b,c,d])", [{"L1": make_list(Atom("a"), Atom("b"))}])
        self.assertQueryTrue("append([a,b], L2, [a,b,c,d])", [{"L2": make_list(Atom("c"), Atom("d"))}])
        self.assertQueryFalse("append(L1, [c,e], [a,b,c,d])") # L1 would need to "fix" the mismatch
        ...
        self.assertQuerySolutions("append(L1, L2, not_a_list)", [
        ...
        self.assertQueryTrue("append([a], [c|d], L3)", [{"L3": Term(Atom("."), [Atom("a"), Term(Atom("."), [Atom("c"), Atom("d")])])}])
