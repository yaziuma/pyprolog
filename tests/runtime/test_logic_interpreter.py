...
class TestLogicInterpreter:
    ...
    def test_unification_basic(self):
        ...
        # アトム同士の単一化
        success, env = self.logic_interpreter.unify(
            Atom("hello"), Atom("hello"), self.env
        )
        assert success

        # 異なるアトムの単一化失敗
        success, env = self.logic_interpreter.unify(
            Atom("hello"), Atom("world"), self.env
        )
        assert not success

        # 変数とアトムの単一化
        success, env = self.logic_interpreter.unify(
            Variable("X"), Atom("value"), self.env
        )
        assert success
    ...
    def test_variable_renaming(self):
        """変数リネームのテスト"""
        rule = Rule(
            Term(Atom("p"), [Variable("X")]),
            Term(Atom("q"), [Variable("X"), Variable("Y")]),
        )
        ...
        assert renamed1_head_x_name == renamed1_body_x_name, (
            "Same original variable 'X' should have same renamed name within a rule"
        )
        assert renamed1_head_x_name != original_x_name, (
            "Renamed 'X' should be different from original 'X'"
        )
        assert renamed1_body_y_name != original_y_name, (
            "Renamed 'Y' should be different from original 'Y'"
        )
        ...
        assert renamed1_head_x_name != renamed2_head_x_name, (
            "Renamed 'X' in first call should be different from renamed 'X' in second call"
        )
        assert renamed1_body_y_name != renamed2_body_y_name, (
            "Renamed 'Y' in first call should be different from renamed 'Y' in second call"
        )

    def test_variable_renaming_consistency(self):
        """変数リネームの一貫性テスト：ルール内で同じ変数は同じ新名にリネームされる"""
        rule = Rule(
            Term(Atom("p"), [Variable("X"), Variable("Y")]),
        ...
        assert r_head_X.name == r_body_X1.name, (
            "All instances of X in head and body should rename to the same variable name"
        )
        assert r_head_X.name == r_body_X2.name, (
            "All instances of X in head and body should rename to the same variable name"
        )
        ...
        assert r_head_Y.name != r_body_Z.name, (
    ...
    def test_goal_resolution(self):
        ...
        assert isinstance(results[0], BindingEnvironment), (
            "Result should be a BindingEnvironment instance"
        )
    ...
    def test_goal_resolution_with_variables(self):
        ...
        results_success = list(
            self.logic_interpreter.solve_goal(goal_success, env_success)
        )

        assert len(results_success) == 1, (
            "Should find one solution for capital(france, X)"
        )
        assert results_success[0].get_value("X") == Atom("paris"), (
            "X should be bound to 'paris'"
        )
        ...
        assert len(results_fail) == 0, (
            "Should find no solutions for capital(germany, Y)"
        )
    ...
    def test_rule_application(self):
        ...
        assert isinstance(results[0], BindingEnvironment), (
            "Result should be a BindingEnvironment"
        )
    ...
    def test_dereference(self):
        ...
        assert self.logic_interpreter.dereference(Variable("X"), env) == Atom(
            "final_value"
        ), "Dereference X -> Y -> Z -> final_value"
        ...
        assert self.logic_interpreter.dereference(Variable("A"), env) == Variable(
            "A"
        ), "Dereferencing unbound variable 'A' should return 'A'"

        # Test 3: Dereferencing a non-variable (Atom)
        assert self.logic_interpreter.dereference(Atom("hello"), env) == Atom(
            "hello"
        ), "Dereferencing an Atom should return the Atom itself"

        # Test 4: Dereferencing a variable bound to an Atom directly
        env.bind("K", Atom("direct_atom"))
        assert self.logic_interpreter.dereference(Variable("K"), env) == Atom(
            "direct_atom"
        ), "Dereference K -> direct_atom"
    ...
    def test_dereference_complex_chain(self):
        ...
        if isinstance(deref_X, Term) and deref_X.args:
            ...
            deref_Y_from_X = self.logic_interpreter.dereference(
                deref_X.args[0], env
            )  # deref_X.args[0] is Y
            ...
            if isinstance(deref_Y_from_X, Term) and len(deref_Y_from_X.args) > 1:
                ...
                deref_A_from_Y = self.logic_interpreter.dereference(
                    deref_Y_from_X.args[1], env
                )  # deref_Y_from_X.args[1] is A
    ...
    def test_dereference_term(self):
        ...
        original_term = Term(
            Atom("myterm"),
            [Variable("A"), Variable("B"), Atom("fixed"), Variable("UNBOUND")],
        )
        ...
        expected_term = Term(
            Atom("myterm"),
            [Atom("val_A"), Atom("val_C"), Atom("fixed"), Variable("UNBOUND")],
        )
        assert reconstructed_term == expected_term, (
            "Reconstructed term with dereferenced args does not match expected"
        )
    ...
    def test_partial_dereference(self):
        ...
        assert deref_X == expected_term_for_X, (
            "Dereferencing X should yield the term data(A,B,C)"
        )
        ...
        d_arg0 = self.logic_interpreter.dereference(
            deref_X.args[0], env
        )  # This is Variable("A")
        d_arg1 = self.logic_interpreter.dereference(
            deref_X.args[1], env
        )  # This is Variable("B")
        d_arg2 = self.logic_interpreter.dereference(
            deref_X.args[2], env
        )  # This is Variable("C")
    ...
    def test_circular_reference_detection(self):
        ...
        s2, e2 = self.logic_interpreter.unify(
            VY, VX, e1
    ...
    def test_unification_failure_rollback(self):
        ...
        success_first, env_after_first_unify = self.logic_interpreter.unify(
            term1_X_Y, term2_fixed_Z, env_before_first_unify
        )
        ...
        success_fail, env_returned_after_fail = self.logic_interpreter.unify(
            term_X_const1, term_valX_const2, env_after_first_unify
        )

        assert not success_fail, (
            "Second unification id_check(X, const1) = id_check(val_X, const2) should fail"
        )
        ...
        assert env_returned_after_fail is env_after_first_unify, (
            "Environment instance should be the same as passed in on failure"
        )

        # Verify that bindings from *before* the failing call are intact and no partial bindings from the failed call exist.
        assert self.logic_interpreter.dereference(
            Variable("A"), env_returned_after_fail
        ) == Atom("original_A_value")
        assert self.logic_interpreter.dereference(X, env_returned_after_fail) == Atom(
            "val_X"
        )
    ...
    def test_complex_term_unification(self):
        ...
        assert self.logic_interpreter.dereference(X, new_env) == Atom("b"), (
            "X should be bound to b"
        )
        assert self.logic_interpreter.dereference(W, new_env) == Atom("a"), (
        ...
        assert self.logic_interpreter.dereference(Y, new_env) == Z, (
            "Y should be bound to Z"
        )
    ...
    def test_list_unification(self):
        ...
        success1, env1 = self.logic_interpreter.unify(
            pattern_H_T, list_abc, BindingEnvironment()
        )
        assert success1, "Unification [H|T] = [a,b,c] failed"
        assert self.logic_interpreter.dereference(Variable("H"), env1) == Atom("a"), (
            "H should be 'a'"
        )
        assert self.logic_interpreter.dereference(Variable("T"), env1) == Term(
            Atom("."), [Atom("b"), Term(Atom("."), [Atom("c"), Atom("[]")])]
        ), "T should be [b,c]"

        # Test case 2: Unifying two identical concrete lists: [a] = [a]
        list_a1 = Term(Atom("."), [Atom("a"), Atom("[]")])
        list_a2 = Term(Atom("."), [Atom("a"), Atom("[]")])
        success2, env2 = self.logic_interpreter.unify(
            list_a1, list_a2, BindingEnvironment()
        )
        ...
        success3, env3 = self.logic_interpreter.unify(
            list_a1, atom_not_list, BindingEnvironment()
        )
        ...
        s4a, e4a = self.logic_interpreter.unify(
            X, concrete_list_v, BindingEnvironment()
        )
        assert s4a, "Unification X = [v] failed"
        assert self.logic_interpreter.dereference(X, e4a) == concrete_list_v, (
            "X should be bound to [v]"
        )

        Y = Variable("Y")
        s4b, e4b = self.logic_interpreter.unify(
            concrete_list_v, Y, BindingEnvironment()
        )
        assert s4b, "Unification [v] = Y failed"
        assert self.logic_interpreter.dereference(Y, e4b) == concrete_list_v, (
            "Y should be bound to [v]"
        )

        # Test case 5: Unifying [X] = [a]
        list_X_nil = Term(Atom("."), [Variable("X_val"), Atom("[]")])
        list_a_nil = Term(Atom("."), [Atom("a_val"), Atom("[]")])
        success5, env5 = self.logic_interpreter.unify(
            list_X_nil, list_a_nil, BindingEnvironment()
        )
        assert success5, "Unification [X_val] = [a_val] failed"
        assert self.logic_interpreter.dereference(Variable("X_val"), env5) == Atom(
            "a_val"
        )
