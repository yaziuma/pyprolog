...
class TestRuntime:
        ...
        if expected_bindings_list is None:  # Only check for success
            ...
            for i, expected_bindings in enumerate(expected_bindings_list):
                ...
                for var_name_str, expected_value in expected_bindings.items():
                    var_key = Variable(
                        var_name_str
                    )  # Query results use Variable objects as keys
    ...
    def test_basic_fact_queries(self):
        ...
        self.assertQueryTrue(
            "likes(john, mary)", [{}]
        )  # Succeeds with one solution, no variables to bind
    ...
    def test_rule_resolution(self):
        ...
        self.assertQueryTrue(
            "grandparent(GP, GC)", [{"GP": Atom("anne"), "GC": Atom("charles")}]
        )  # Query with variables
        self.assertQueryFalse("grandparent(bob, anne)")
        self.assertQueryTrue(
            "grandparent(anne, charles)", [{}]
        )  # Query with atoms, expect success no bindings
    ...
    def test_comparison_operations(self):
        ...
        self.assertQueryTrue(
            "X is 2, Y is 4, X < Y", [{"X": Number(2), "Y": Number(4)}]
        )
    ...
    def test_control_flow(self):
        ...
        assert solutions[0].get(Variable("X")) == Number(1), (
            "X should be 1 for q(X) with cut"
        )
    ...
    def test_variable_unification(self):
        ...
        self.assertQueryTrue(
            "person(Name, Age)", [{"Name": Atom("john"), "Age": Number(25)}]
        )
    ...
    def test_dynamic_predicates(self):
        ...
        assert (
            len(solutions_my_fact) == 2
        )  # my_fact(1) from asserta, my_fact(2) from assertz
        ...
        self.assertQueryTrue(
            "retract(my_fact(1)).", [{}]
        )  # Retract might return bindings for the retracted clause
        self.assertQueryTrue(
    ...
    def test_error_handling(self):
        ...
        assert self.runtime.query("a :- .") == [], (
            "Query with syntax error should return empty list"
        )
        assert self.runtime.query("X is 1/0.") == [], (
            "Query with arithmetic error (div by zero) should return empty"
        )
    ...
    def test_multiple_solutions(
        self,
    ):  # Covered by test_backtracking in logic_interpreter and here
        """複数解のテスト"""
        self._skip_if_not_implemented()
    ...
    def test_term_manipulation(self):
        ...
        # Analysis mode
        self.assertQueryTrue(
            "functor(f(a,b), F, A)", [{"F": Atom("f"), "A": Number(2)}]
        )
        self.assertQueryTrue(
            "functor(atom, F, A)", [{"F": Atom("atom"), "A": Number(0)}]
        )
        self.assertQueryTrue(
            "functor(123, F, A)", [{"F": Number(123), "A": Number(0)}]
        )  # Numbers are atomic, functor is the number itself, arity 0
        self.assertQueryTrue(
            "functor([a], F, A)", [{"F": Atom("."), "A": Number(2)}]
        )  # List [a] is .(a, [])
        self.assertQueryTrue(
            "functor([], F, A)", [{"F": Atom("[]"), "A": Number(0)}]
        ...
        assert solutions_var_functor[0].get(Variable("T")).functor == Atom(
            "variable_as_functor"
        )
        ...
        self.assertQueryFalse(
            "functor(T, f(a), 2)"
        )  # Functor must be atom or number, not a compound term
        self.assertQueryFalse(
            "functor(f(a), 'g', 1)"
        )  # Analysis mode, but functor/arity don't match
        ...
        self.assertQueryTrue(
            "arg(1, [x,y], H)", [{"H": Atom("x")}]
        )  # H = x for list [x,y] = .(x, .(y,[]))
        ...
        self.assertQueryTrue(
            "atom =.. L", [{"L": Term(Atom("."), [Atom("atom"), Atom("[]")])}]
        )  # L = [atom]
        self.assertQueryTrue(
            "123 =.. L", [{"L": Term(Atom("."), [Number(123), Atom("[]")])}]
        )  # L = [123]
        self.assertQueryTrue(
            "[a,b] =.. L",
            [
        ...
        self.assertQueryTrue(
            "[] =.. L", [{"L": Term(Atom("."), [Atom("[]"), Atom("[]")])}]
        )  # L = [[]]

        # Construction mode
        # T =.. [foo, a, b]  => T = foo(a,b)
        self.assertQueryTrue(
            "T =.. [foo, a, b]", [{"T": Term(Atom("foo"), [Atom("a"), Atom("b")])}]
        )
        # T =.. [atom]  => T = atom
        ...
        self.assertQueryTrue(
            "T =.. ['.', a, b]", [{"T": Term(Atom("."), [Atom("a"), Atom("b")])}]
        )
        # T =.. ['.', a, [b]] => T = [a,b] (same as .(a,.(b,[])))
        list_term = Term(
            Atom("."), [Atom("a"), Term(Atom("."), [Atom("b"), Atom("[]")])]
        )
        ...
        self.assertQueryFalse(
            "T =.. [Var, b]"
        )  # Functor must be an instantiated atom or number, not a variable
        self.assertQueryFalse("T =.. not_a_list")  # Right side must be a list
        self.assertQueryFalse(
            "T =.. [foo | not_a_list_tail]"
        )  # Right side must be a proper list
        self.assertQueryFalse(
            "f(X) =.. [f,a,b]"
        )  # Arity mismatch (Term has arity 1, List implies arity 2)
        self.assertQueryFalse("f(a,b) =.. [f,a]")  # Arity mismatch
        self.assertQueryFalse(
            "atom =.. [atom, extra]"
        )  # Arity mismatch (atom has arity 0)
        self.assertQueryFalse("1 =.. [1,2,3]")  # Arity mismatch (number has arity 0)
        self.assertQueryFalse(
            "T =.. [foo,a,b,c,d,e,f,g,h,i,j,k,l,m,n,o,p,q,r,s,t,u,v,w,x,y,z,aa,ab,ac,ad,ae,af,ag,ah,ai,aj,ak,al,am,an,ao,ap,aq,ar,as,at,au,av,aw,ax,ay,az,ba]"
        )  # Arity too large (implementation defined limit, usually 255 or so) - this specific test might depend on MAX_ARITY in your implementation. Assuming it's less than this.
    ...
    def test_type_checking(self):
        ...
        self.assertQueryTrue(
            "var(X), Y=X, var(Y)", [{"X": Variable("X"), "Y": Variable("X")}]
        )  # Y is bound to X, both are vars
