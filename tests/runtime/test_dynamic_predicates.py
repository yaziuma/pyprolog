...
class TestDynamicPredicates(unittest.TestCase):
    ...
    def assertQueryTrue(self, query_string, expected_bindings_list=None, msg=None):
        ...
        if expected_bindings_list is None:
            self.assertGreaterEqual(len(solutions), 1, msg or f"Query '{query_string}' should succeed but failed (no solutions).")
    ...
    def assertQueryFalse(self, query_string, msg=None):
        solutions = self.runtime.query(query_string)
        self.assertEqual(len(solutions), 0, msg or f"Query '{query_string}' should fail but succeeded with {len(solutions)} solution(s).")
    ...
    def assertPrologError(self, query_string, error_type=PrologError, msg=None):
        ...
        solutions = self.runtime.query(query_string)
        self.assertEqual(len(solutions), 0, msg or f"Query '{query_string}' expected to fail due to error, but returned solutions.")
    ...
    def test_asserta_fact(self):
        ...
        self.assertEqual(sols_p1[0].get(Variable("X")), Number(1.0)) # Parser creates Number(1.0)

        self.assertQueryTrue("asserta(p(0)).", [{}])
        # p(0) が先頭に追加され、p(1) が次にくることを確認
        sols_p2 = self.runtime.query("p(X)")
        self.assertEqual(len(sols_p2), 2)
        self.assertEqual(sols_p2[0].get(Variable("X")), Number(0.0)) # asserta は先頭に追加
    ...
    def test_assertz_fact(self):
        ...
        sols_p2 = self.runtime.query("p(X)")
        self.assertEqual(len(sols_p2), 2)
        self.assertEqual(sols_p2[0].get(Variable("X")), Number(1.0)) # assertz は末尾に追加
    ...
    def test_assert_with_variables(self):
        ...
        self.assertIsInstance(solutions1[0].get(Variable("Y")), Variable, "Y should be a variable")
        ...
        self.assertIsInstance(solutions2[0].get(Variable("Z")), Variable, "Z should be a variable")
        ...
        if hasattr(var_A, 'name') and hasattr(var_B, 'name'): # Make sure they are Variable objects
             self.assertNotEqual(id(var_A), id(var_B), "Variables from different asserta should be different objects/scopes")

        # Test binding one and checking the other (if they were accidentally the same instance)
        self.assertQueryTrue("v_holder(first_val).", [{}]) # Binds Var in v_holder to first_val
        sol_v3 = self.runtime.query("another_v(C)") # C should be unified with Var from another_v
        self.assertIsInstance(sol_v3[0].get(Variable("C")), Variable, "Var in another_v should remain unbound by v_holder(first_val)")
