...
class TestMetaPredicates:
    ...
    def assertQuerySolutions(self, query_string: str, expected_solutions_list: list, msg: str = None):
    ...
    def assertQueryTrue(self, query_string, expected_bindings_list=None, msg=None):
        ...
        if expected_bindings_list is None or not expected_bindings_list:
            ...
            for var_key, value in first_solution_bindings.items():
                if isinstance(var_key, Variable): # Ensure keys are variable names (str)
    ...
    def test_findall_template_multiple_variables(self):
        ...
        self.assertQueryTrue(
            "findall(item(Num, Name), s(Num, Name), List)",
            [{"List": expected_list}]
        )
    ...
    def test_findall_complex_goal_conjunction(self):
        ...
        expected_elements = [
            Term(Atom("t"), [Number(1), Atom("a")]), Term(Atom("t"), [Number(1), Atom("b")]),
            Term(Atom("t"), [Number(2), Atom("a")]), Term(Atom("t"), [Number(2), Atom("b")]),
            Term(Atom("t"), [Number(3), Atom("a")]), Term(Atom("t"), [Number(3), Atom("b")])
        ]
        self.assertQueryTrue(
    ...
    def test_findall_goal_throws_exception(self):
        ...
        assert self.runtime.add_rule(error_rule_str), "Failed to add error-generating rule"

        query_str_for_log = "findall(X, error_generating_goal_unique123, List)"
        print(f"PYTHON_PRINT_ASSERT: Querying for exception test: '{query_str_for_log}'", flush=True)
        ...
        findall_goal_term = Term(Atom("findall"), [
            Variable("X"),
            Atom("error_generating_goal_unique123"),
            Variable("List")
        ])
        ...
        with pytest.raises(PrologError):
            ...
            list(self.runtime.execute(findall_goal_term, BindingEnvironment())) # Use fresh env
    ...
    def test_findall_goal_not_callable_number(self):
        ...
        with pytest.raises(PrologError, match="type_error\\(callable, 123\\)"): # Match integer form
             # Using direct execute to test exception propagation
            findall_goal_term = Term(Atom("findall"), [
                Variable("X"),
    ...
    def test_findall_order_of_solutions(self):
        ...
        self.assertQueryTrue(
            ...
            [{"OrderedList": self._make_prolog_list([Atom("x"), Atom("y"), Atom("z")])}]
        )
    ...
    def test_findall_template_vars_not_in_goal(self):
        ...
        assert py_list[0] == Term(Atom("t"), [Number(10), Variable("Y")]) # Assuming 'Y' is the name from template
        ...
        self.assertQueryTrue(query_str, [{"List": self._make_prolog_list([
