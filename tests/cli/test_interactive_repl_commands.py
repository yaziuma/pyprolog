...
"""
    ...
    def test_explain_command_basic(self):
        ...
        with patch("builtins.print") as mock_print:
            ...
            explanation_found = any(
                "Query Explanation" in str(arg) or "parent(tom, mary)" in str(arg)
    ...
    def test_explain_command_no_args(self):
        ...
        with patch("builtins.print") as mock_print:
            ...
            # エラーメッセージが表示されることを確認
            error_found = any(
                "使用法" in str(call) or "Usage" in str(call)
    ...
    def test_search_command_basic(self):
        ...
        with patch("builtins.print") as mock_print:
            ...
            printed_args = [call[0][0] for call in mock_print.call_args_list if call[0]]
            search_found = any(
                "検索結果" in str(arg) or "Search" in str(arg) or "parent" in str(arg)
                for arg in printed_args
            )
            assert search_found
    ...
    def test_search_stats_command(self):
        ...
        with patch("builtins.print") as mock_print:
            ...
            stats_found = any(
                "統計" in str(arg)
                or "Statistics" in str(arg)
                or "インデックス" in str(arg)
                for arg in printed_args
            )
            assert stats_found
    ...
    def test_validate_command_basic(self):
        ...
        with patch("builtins.print") as mock_print:
            ...
            printed_args = [call[0][0] for call in mock_print.call_args_list if call[0]]
            validate_found = any(
                "検証" in str(arg) or "Validation" in str(arg) or "解析" in str(arg)
                for arg in printed_args
            )
    ...
    def test_validate_stats_command(self):
        ...
        with patch("builtins.print") as mock_print:
            ...
            printed_args = [call[0][0] for call in mock_print.call_args_list if call[0]]
            stats_found = any(
                "統計" in str(arg) or "Statistics" in str(arg) or "シンボル" in str(arg)
                for arg in printed_args
            )
            assert stats_found
    ...
    def test_commands_without_runtime(self):
        ...
        with patch("builtins.print") as mock_print:
            ...
            runtime_error = any(
                "ランタイム" in msg or "Runtime" in msg or "初期化" in msg
    ...
    def test_command_error_handling(self):
        ...
        with patch("builtins.print") as mock_print:
            ...
            error_handled = (
                "エラー" in printed_output
                or "Error" in printed_output
                or "error" in printed_output
            )
