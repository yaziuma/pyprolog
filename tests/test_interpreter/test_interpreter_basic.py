...
from prolog.core_types import Variable, Term, Rule, TRUE_TERM, FALSE_TERM, CUT_SIGNAL, Conjunction
...
def test_multi_term_query():
    ...
    for i, bindings_dict in enumerate(runtime.query(goal_text)):
        solutions_found += 1
        assert R_var in bindings_dict, f"Variable R not in bindings: {bindings_dict} (solution {i+1})"
        assert T_var in bindings_dict, f"Variable T not in bindings: {bindings_dict} (solution {i+1})"
        ...
        assert bound_r == expected_r, f"Binding for R was '{bound_r}', expected '{expected_r}' (solution {i+1})"
        assert bound_t == expected_t, f"Binding for T was '{bound_t}', expected '{expected_t}' (solution {i+1})"

    assert solutions_found == len(expected_bindings_list), f"Expected {len(expected_bindings_list)} solutions, got {solutions_found}"
...
def test_support_for_string_literals():
    ...
    expected_bindings_list = [
        {'Y': 'chicago', 'Z': 'good_credit'}
    ]
    
    solutions_found = 0
    for i, bindings_dict in enumerate(runtime.query(goal_text)):
        solutions_found += 1
        assert Y_var in bindings_dict, f"Variable Y not in bindings: {bindings_dict} (solution {i+1})"
        assert Z_var in bindings_dict, f"Variable Z not in bindings: {bindings_dict} (solution {i+1})"
        ...
        assert bound_y == expected_y, f"Binding for Y was '{bound_y}', expected '{expected_y}' (solution {i+1})"
        assert bound_z == expected_z, f"Binding for Z was '{bound_z}', expected '{expected_z}' (solution {i+1})"
