...
class IOPredicate(BuiltinPredicate, ABC):
    ...
    # 共通実装（テンプレートメソッドパターン）
    def execute(self, runtime: Mock, env: BindingEnvironment) -> Iterator[BindingEnvironment]:
        """共通実行フロー"""
        try:
...
class TestIOPredicateBase:
    ...
    def test_argument_validation_failure(self):
        ...
        with pytest.raises(PrologError, match=r"get_char/1 expects 1 argument\(s\), got 0"):
            TestGetCharPredicate()
        
        with pytest.raises(PrologError, match=r"get_char/1 expects 1 argument\(s\), got 2"):
    ...
    def test_unify_with_argument(self):
        ...
        target_term = Atom("a")
        results = list(predicate._unify_with_argument(mock_runtime, mock_env, target_term))
