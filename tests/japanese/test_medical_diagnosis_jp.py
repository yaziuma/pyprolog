...
from tests.conftest import TestFixtures, assert_query_succeeds, assert_query_fails
...
class TestMedicalDiagnosisJapanese:
    ...
    def test_basic_symptom_facts(self):
        ...
        # 症状の存在を確認
        test_symptoms = ['発熱', '咳', 'のどの痛み', '鼻水', '体の痛み', '頭痛', '息切れ', '味覚消失', '嗅覚消失']
    ...
    def test_disease_symptom_relations(self):
        ...
        for disease, symptom, probability in test_relations:
            ...
            try:
                ...
                assert len(results) > 0, f"関連性が見つかりません: {disease} - {symptom}"
            except Exception as e:
    ...
    def test_risk_factors(self):
        ...
        for condition, disease, factor in test_risk_factors:
            ...
            try:
                ...
                assert len(results) > 0, f"リスク要因が見つかりません: {condition} -> {disease}"
            except Exception as e:
    ...
    def test_age_categories(self):
        ...
        for age, expected_category in test_cases:
            ...
            try:
                ...
                assert len(results) > 0, f"年齢カテゴリが正しくありません: {age} -> {expected_category}"
            except Exception as e:
    ...
    def test_season_factors(self):
        ...
        for season, disease, factor in test_season_factors:
            ...
            try:
                ...
                assert len(results) > 0, f"季節要因が見つかりません: {season} -> {disease}"
            except Exception as e:
    ...
    def test_comprehensive_patient_diagnosis(self):
        ...
        for i, case in enumerate(test_diagnosis_cases, 1):
            query = f"患者診断({case['symptoms']}, {case['age']}, {case['history']}, 結果)"
            try:
                results = self.runtime.query(query)
                ...
                if len(results) > 0:
                    print(f"  診断が正常に実行されました")
                else:
                    print(f"  警告: 診断結果が返されませんでした")
    ...
    def test_auxiliary_predicates(self):
        ...
        for input_list, expected_result in test_cases:
            ...
            try:
                ...
                if len(results) > 0:
                    print(f"  リスト乗算が正しく計算されました: {input_list} -> {expected_result}")
            except Exception as e:
    ...
    def test_japanese_variable_support(self):
        ...
        for query in japanese_variable_queries:
            try:
                ...
                if len(results) > 0:
                    print(f"  日本語変数名が正しく処理されました")
                else:
                    print(f"  注意: 解が見つかりませんでした（パーサーの制限の可能性）")
            except Exception as e:
    ...
    def test_edge_cases_and_error_handling(self):
        ...
        for query in invalid_queries:
            try:
                ...
                assert len(results) == 0, f"存在しないデータに対してfalseが返されるべき: {query}"
            except Exception as e:
