...
class TestScanner:
    ...
    def test_japanese_atoms(self):
        ...
        for i, token in enumerate(tokens):
            assert token.lexeme == expected_lexemes[i], (
                f"Token {i} lexeme mismatch: expected '{expected_lexemes[i]}', got '{token.lexeme}'"
            )
            assert token.token_type == expected_types[i], (
                f"Token {i} type mismatch: expected {expected_types[i]}, got {token.token_type}"
            )
