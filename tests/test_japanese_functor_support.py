...
class TestJapaneseFunctorSupport(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.runtime = Runtime(
            variable_mapper=self.variable_mapper, functor_mapper=self.functor_mapper
        )
    ...
    def test_unicode_character_sets(self):
        ...
        for char, lang in test_cases:
            # マッピング必要性確認
            self.assertTrue(
                mapper.needs_mapping(char),
                f"{lang}文字 '{char}' のマッピングが必要と判定されない",
            )
            ...
            self.assertTrue(
                mapped.startswith("MAPPED_F"),
                f"{lang}文字のマッピング形式が不正: {mapped}",
            )
            ...
            self.assertEqual(
                recovered,
                char,
                f"{lang}文字の逆マッピングが失敗: {char} -> {mapped} -> {recovered}",
            )
    ...
    def test_mapping_consistency(self):
        ...
        self.assertEqual(
            mapped1, mapped2, "同一ファンクターのマッピングが一貫していない"
        )
        ...
        # （決定論的なマッピング生成）
        self.assertEqual(
            mapped1, mapped3, "異なるインスタンスで異なるマッピングが生成された"
        )
