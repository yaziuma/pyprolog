...
class TestValidationResult:
    ...
    def test_validation_result_creation(self):
        ...
        issues = [
            ValidationIssue("error_type", "error", "エラーメッセージ", test_fact, None, 1, 0),
            ValidationIssue("warning_type", "warning", "警告メッセージ", test_fact, None, 2, 0),
            ValidationIssue("info_type", "info", "情報メッセージ", test_fact, None, 3, 0)
        ]
    ...
    def test_has_warnings_method(self):
        ...
        # 警告なしのケース
        info_issues = [
            ValidationIssue("info_type", "info", "情報", None, None, 1, 0)
        ]
    ...
    def test_validation_result_with_large_dataset(self):
        ...
        for i in range(1000):
            ...
            large_issues.append(
                ValidationIssue(f"type_{i % 10}", issue_type, f"メッセージ{i}", None, None, i, 0)
            )
        
        result = ValidationResult(large_issues, 1000, 5.0)
        
        assert len(result.issues) == 1000
        assert result.get_error_count() + result.get_warning_count() + result.get_info_count() == 1000
