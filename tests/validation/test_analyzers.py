...
class TestReachabilityAnalyzer:
    ...
    def setup_method(self):
        ...
        # 到達可能なルール
        reachable_rule = Rule(
            Term(Atom("reachable"), [Variable("X")]), Term(Atom("entry_point"), [])
        )
        ...
        self.symbol_table.add_predicate(
            "unreachable", 1, unreachable_rule, "test.pl", 3
        )
    ...
    def test_is_entry_point(self):
        ...
        fact_info = type(
            "PredicateInfo",
            (),
            {"definition": Fact(Term(Atom("test"), [])), "arity": 0},
        )()
...
class TestUndefinedAnalyzer:
    ...
    def setup_method(self):
        ...
        self.symbol_table.add_reference(
            "undefined_pred", 1, undefined_rule, "test.pl", 2, "body"
        )
    ...
    def test_find_similar_predicates(self):
        ...
        # 類似の述語を検索
        similar = self.analyzer._find_similar_predicates(
            "defined", 1, defined_predicates
        )
...
class TestAnalyzersIntegration:
    ...
    def setup_method(self):
        ...
        # 正常なルール
        normal_rule = Rule(
            Term(Atom("process"), [Variable("X")]), Term(Atom("main"), [])
        )
        ...
        self.symbol_table.add_predicate(
            "problematic", 1, problematic_rule, "main.pl", 3
        )
        self.symbol_table.add_reference(
            "missing_predicate", 1, problematic_rule, "main.pl", 3, "body"
        )
    ...
    def test_all_analyzers_run_without_error(self):
        ...
        # すべてのアナライザーを実行
        conflict_issues = self.conflict_analyzer.analyze(
            self.symbol_table, self.dependency_graph
        )
        reachability_issues = self.reachability_analyzer.analyze(
            self.symbol_table, self.dependency_graph
        )
        undefined_issues = self.undefined_analyzer.analyze(
            self.symbol_table, self.dependency_graph
        )
    ...
    def test_analyzer_performance_with_large_dataset(self):
        ...
        start = time.time()
        conflict_issues = self.conflict_analyzer.analyze(
            large_symbol_table, large_dependency_graph
        )
