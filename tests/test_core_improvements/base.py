...
class BaseTestCore:
    ...
    def _consult(self, rules_str):
        logger.info(
            f"Consulting rules: '{rules_str[:100]}{'...' if len(rules_str) > 100 else ''}'"
        )
        try:
            ...
            logger.debug(
                f"Rules consulted. Current rule count: {len(self.runtime.rules)}"
            )
        except Exception as e:
            logger.error(
                f"Consult '{rules_str}' raised an exception: {e}", exc_info=True
            )
    ...
    def _assert_true(self, query_str, expected_bindings_list=None, exact_order=False):
        logger.info(
            f"Asserting true for query: '{query_str}' with expected_bindings: {expected_bindings_list}, exact_order={exact_order}"
        )
        solutions = self._query(query_str)

        if (
            expected_bindings_list is None
        ):  # Expecting success, at least one solution, bindings don't matter
            if not solutions:
                logger.error(
                    f"Query '{query_str}' failed (no solutions). Expected success (any solution)."
                )
                pytest.fail(
                    f"Query '{query_str}' failed (no solutions). Expected success (any solution)."
                )
        ...
        ):  # Expecting success with no specific variable bindings (e.g. ground query true)
            if not (
                len(solutions) == 1 and solutions[0] == {}
            ):  # Check for exactly one empty dict solution
                logger.error(
                    f"Query '{query_str}' did not yield exactly one empty binding. Got: {solutions}. Expected success (ground query)."
                )
                pytest.fail(
                    f"Query '{query_str}' did not yield exactly one empty binding. Got: {solutions}. Expected success (ground query)."
                )
        ...
        if (
            not solutions and expected_bindings_list
        ):  # This condition implies expected_bindings_list is not None and not []
            logger.error(
                f"Query '{query_str}' failed (no solutions). Expected success with bindings: {expected_bindings_list}"
            )
            pytest.fail(
                f"Query '{query_str}' failed (no solutions). Expected success with bindings: {expected_bindings_list}"
            )
        ...
        for sol_binding_map in solutions:
            ...
            if not isinstance(sol_binding_map, dict):
                logger.error(
                    f"Solution for '{query_str}' is not a dict: {sol_binding_map}"
                )
                pytest.fail(
                    f"Solution for '{query_str}' is not a dict: {sol_binding_map}"
                )
            for var_obj, val_term in sol_binding_map.items():
                if not isinstance(var_obj, Variable):
                    logger.error(
                        f"Key in solution binding for '{query_str}' is not a Variable: {var_obj}"
                    )
                    pytest.fail(
                        f"Key in solution binding for '{query_str}' is not a Variable: {var_obj}"
                    )
                ...
                if isinstance(val_term, Number):
                    ...
                    processed_sol[var_obj.name] = str(
                        val_term
                    )  # For complex terms or other types
            processed_solutions.append(processed_sol)
        logger.debug(
            f"Processed actual solutions for '{query_str}': {processed_solutions}"
        )
        ...
        logger.debug(
            f"Processed expected bindings for '{query_str}': {processed_expected}"
        )
    ...
    def _assert_false(self, query_str):
        ...
        if not solutions:
            ...
            logger.error(
                f"Query '{query_str}' succeeded, expected failure. Solutions: {solutions}"
            )
            pytest.fail(
                f"Query '{query_str}' succeeded (solutions: {solutions}), expected failure."
            )
