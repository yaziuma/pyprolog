...
class TestFixtures:
    ...
    def temp_prolog_file():
        ...
        temp_file = tempfile.NamedTemporaryFile(
            mode="w", suffix=".prolog", delete=False
        )
        temp_file.write("""
        test_fact(a).
        test_fact(b).
...
class TestDataGenerator:
    ...
    def generate_complex_terms(complexity: int = 5):
        ...
        if complexity <= 1:
            ...
            sub_terms = [
                TestDataGenerator.generate_complex_terms(complexity - 1)
                for _ in range(2)
            ]
...
def should_skip_test(func):
    ...
    if hasattr(func, "_slow_test") and TEST_CONFIG["skip_slow_tests"]:
        return True
    if (
        hasattr(func, "_memory_intensive")
        and TEST_CONFIG["skip_memory_intensive_tests"]
    ):
        return True
