...
class TestListingExportIntegration:
    ...
    def setup_method(self):
        ...
        self.runtime.add_rule("works_on(Person, Project) :- person(Person, _, Dept), project(Project, Dept, active).")
    ...
    def test_export_facts_only_exports_facts(self):
        ...
        # person/3をエクスポート
        solutions = list(self.runtime.query(f"export_facts(person/3, '{output_file}')."))
        assert len(solutions) == 1
    ...
    def test_listing_and_export_consistency(self):
        ...
        solutions = list(self.runtime.query(f"export_facts(department/2, json('{export_file}'))."))
        assert len(solutions) == 1
    ...
    def test_multiple_predicate_export(self):
        ...
        # 各述語を個別にエクスポート
        solutions1 = list(self.runtime.query(f"export_facts(person/3, '{person_file}')."))
        solutions2 = list(self.runtime.query(f"export_facts(department/2, '{dept_file}')."))
        solutions3 = list(self.runtime.query(f"export_facts(project/3, json('{project_file}'))."))
    ...
    def test_export_after_dynamic_changes(self):
        ...
        solutions = list(self.runtime.query(f"export_facts(person/3, json('{initial_file}'))."))
        assert len(solutions) == 1
        ...
        solutions = list(self.runtime.query(f"export_facts(person/3, json('{updated_file}'))."))
        assert len(solutions) == 1
    ...
    def test_error_recovery_in_mixed_operations(self):
        ...
        solutions = list(self.runtime.query(f"export_facts(person/3, '{invalid_path}')."))
    ...
    def test_japanese_integration(self):
        ...
        solutions = list(self.runtime.query(f"export_facts(社員/3, json('{output_file}'))."))
