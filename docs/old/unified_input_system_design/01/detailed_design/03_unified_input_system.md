# 統一入力システム（Unified Input System）詳細設計
...
### 1.1 責任
- 入力要求の中央制御・ルーティング
- InputHandlerとThreadingControllerの統合管理
- **フォールバック機能（Fallback Mechanism）**の提供
- 実行モード（シングル/マルチスレッド）の制御

### 1.2 位置づけ
```
IOPredicate → IOManager → 統一入力システム（Unified Input System） → InputHandler/Threading
```
...
if TYPE_CHECKING:
    from pyprolog.runtime.io_manager import IOStream
    ...
    def request_input(
        ...
        **kwargs
        ...
        try:
            ...
            self.error_count += 1
            logger.error(f"UnifiedInputSystem error: {e}")
...
```python
# 統一入力システム初期化
unified_input = UnifiedInputSystem()
...
unified_input.set_input_handler(StandardInputHandler())

# シングルスレッドモードでの使用
result = unified_input.request_input("line", "read_line", "名前を入力: ")
```
...
```python
# マルチスレッドモード有効化
unified_input.enable_threading()

# この呼び出しで真の継続実行が発生
# スレッドブロッキングでスタックフレーム保持
result = unified_input.request_input("char", "get_char", "文字入力: ")
```

### 3.3 カスタムInputHandler
...
class GUIInputHandler(InputHandler):
    ...
    def handle_input_request(self, event: InputEvent) -> Optional[str]:
        if event.input_type == "char":
            return self.dialog.get_char()
        elif event.input_type == "line":
            return self.dialog.get_line()
        return None

# GUI統合
gui_handler = GUIInputHandler(my_dialog)
unified_input.set_input_handler(gui_handler)
unified_input.enable_threading()  # GUI応答性向上
```
...
### 4.1 統一性
- 全ての入力タイプが同一インターフェース
- シングル/マルチスレッドモードの透過的切り替え
- 一貫したエラーハンドリング

### 4.2 拡張性  
- 新しい入力タイプの容易な追加
- カスタムInputHandlerによる柔軟な統合
- プラグイン式アーキテクチャ
...
この設計により、pyprologの入力処理が統一され、真の継続実行が実現可能になります。