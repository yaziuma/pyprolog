...
def test_query_with_builtins():
    ...
    X_var = Variable("X")
    expected_bindings_for_x = ["kitchen", "office", "hall", "dinning room", "cellar"]
    ...
    for i, bindings_dict in enumerate(runtime.query(goal_text)):
        ...
        expected_val_raw = expected_bindings_for_x[
            i
        ]  # e.g., 'kitchen' or 'dinning room'
...
def test_cut_predicate():
    ...
    assert solutions_a == expected_solutions_a, (
        f"Expected {expected_solutions_a}, got {solutions_a} for cut_test_a"
    )
    ...
    assert solutions_b == expected_solutions_b, (
        f"Expected {expected_solutions_b}, got {solutions_b} for cut_test_b"
    )
