...
def test_insert_rule_left():  # asserta (direct runtime call)
    ...
    assert room_solutions == expected_room_order, (
        f"Expected order {expected_room_order}, got {room_solutions}"
    )
...
def test_remove_rule_fact():  # retract (direct runtime call for a fact)
    ...
    retracted_count = runtime.retract(
        rule_to_retract_template
    )  # Assuming retract returns number of rules removed or True/False
    ...
    assert retracted_count is True or retracted_count > 0, (
        "retract should indicate success"
    )
    ...
    assert len(list(runtime.query("room(bathroom)."))) == 0, (
        "Retracted rule should not be queryable"
    )
...
def test_remove_complex_rule():  # retract (direct runtime call for a rule with body)
    ...
    retracted_count = runtime.retract(
        template_to_retract
    )  # This will retract the first matching rule for take/1.
    assert retracted_count is True or retracted_count > 0
...
def test_assertz_rule_builtin_context():
    ...
    assert results_after_c == ["a", "b", "c"], (
        f"Expected order ['a', 'b', 'c'], got {results_after_c}"
    )
