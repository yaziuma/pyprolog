...
from pyprolog.validation.validation_result import ValidationResult
...
class ValidateTool:
    ...
    def validate_query(self, check_type: str = "all", detailed: bool = False) -> Dict[str, Any]:
        """
        検証を実行します
    ...
    def format_results(self, validation_result: Dict[str, Any], format_type: str = "text") -> str:
        """
        検証結果をフォーマットします
    ...
    def _format_text(self, validation_result: Dict[str, Any]) -> str:
        ...
        if issues:
            ...
            for i, issue in enumerate(sorted_issues, 1):
                ...
                lines.append(f"{i}. {severity_symbol} [{issue['issue_type']}] {issue['message']}")
                
                if issue.get('file_path'):
                    lines.append(f"   📁 場所: {issue['file_path']}:{issue['line_number']}")
    ...
    def _format_detailed(self, validation_result: Dict[str, Any]) -> str:
        ...
        lines.append("📈 解析統計:")
        lines.append(f"  • 検証タイプ: {validation_result['check_type']}")
        lines.append(f"  • 詳細解析: {'はい' if validation_result.get('detailed') else 'いいえ'}")
        lines.append(f"  • 解析ルール数: {validation_result['total_rules_analyzed']}")
    ...
    def rebuild_analysis(self) -> bool:
        ...
        try:
            ...
            self.validator.dependency_graph = self.validator.dependency_graph.__class__()
