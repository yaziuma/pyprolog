    ...
    def unify_patterns(pattern_term: Term, target_term: Term) -> Tuple[bool, float]:
        ...
        try:
            ...
            if not isinstance(pattern_term, Term) or not isinstance(target_term, Term):
                return str(pattern_term) == str(target_term), 1.0 if str(pattern_term) == str(target_term) else 0.0
            
            # ファンクター名の比較
            pattern_functor = pattern_term.functor.name if hasattr(pattern_term.functor, 'name') else str(pattern_term.functor)
            target_functor = target_term.functor.name if hasattr(target_term.functor, 'name') else str(target_term.functor)
            
            if pattern_functor != target_functor:
                return False, 0.0
            
            # 引数の数の比較
            pattern_args = pattern_term.args if hasattr(pattern_term, 'args') and pattern_term.args else []
            target_args = target_term.args if hasattr(target_term, 'args') and target_term.args else []
