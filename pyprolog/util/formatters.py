...
class PrologFormatter:
    ...
    def format_predicate_rules(self, rules: List[Union[Rule, Fact]], predicate_name: str, arity: int) -> str:
        """
        特定述語のルール・事実のみを整形
    ...
    def _format_compound_term(self, term: Term) -> str:
        ...
        # リスト記法の特別処理
        if isinstance(term.functor, Atom) and term.functor.name == "." and len(term.args) == 2:
    ...
    def _format_list(self, term: Term, collected_elements: Optional[List[str]] = None) -> str:
        """
        リスト構造を[a,b,c]形式で整形
        ...
        if isinstance(term, Term) and isinstance(term.functor, Atom) and term.functor.name == ".":
            # [Head|Tail]構造
            if len(term.args) >= 1:
    ...
    def _get_functor_name(self, functor: PrologType) -> str:
        ...
        if isinstance(functor, Atom):
            ...
            if self.functor_mapper:
                original_name = self.functor_mapper.map_english_to_non_ascii(functor.name)
                return original_name
            return functor.name
        else:
            return str(functor)
    
    def _filter_rules_by_predicate(self, rules: List[Union[Rule, Fact]], predicate_name: str, arity: int) -> List[Union[Rule, Fact]]:
        """
        指定述語のルール・事実をフィルタリング
    ...
    def _matches_predicate(self, term: Term, predicate_name: str, arity: int) -> bool:
        ...
        if not name_match and self.functor_mapper:
            # 日本語→英語マッピング
            mapped_predicate = self.functor_mapper.map_non_ascii_to_english(predicate_name)
            name_match = (term_functor_name == mapped_predicate)
            
            # 英語→日本語マッピング
            if not name_match:
                mapped_term = self.functor_mapper.map_english_to_non_ascii(term_functor_name)
