...
class Scanner:
    ...
    def __init__(self, source: str, report: Callable[[int, str], None]=default_error_handler, variable_mapper: Optional[VariableMapper]=None, functor_mapper: Optional[FunctorMapper]=None):
        self._source = source
        ...
        self._keywords = {'true': TokenType.TRUE, 'fail': TokenType.FAIL, 'retract': TokenType.RETRACT, 'asserta': TokenType.ASSERTA, 'assertz': TokenType.ASSERTZ, 'write': TokenType.ATOM, 'nl': TokenType.ATOM}
        ...
        logger.debug('Scanner initialized with %r operators', len(self._operator_symbols))
    ...
    def _identifier(self):
        """識別子のスキャン（Unicode対応版）"""
        while not self._is_at_end() and (self._peek().isalnum() or self._peek() == '_' or (self._peek() and ord(self._peek()) > 127) or self._is_valid_identifier_char(self._peek())):
            self._advance()
        ...
        if token_type is None:
            if text in self._operator_symbols:
                ...
                logger.debug("Mapped non-ASCII functor '%r' to '%r'", text, literal_override)
                ...
                logger.debug("Mapped Japanese variable '%r' to '%r'", text, literal_override)
