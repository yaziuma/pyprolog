...
class SymbolTable:
    ...
    def add_reference(
        ...
        container_key = predicate_key(
            container_rule_head.functor.name, len(container_rule_head.args)
        )
        if container_key not in self.predicates:
            logger.warning(
                "Attempted to add reference to a container rule '%r' that is not in the symbol table.",
                container_key,
            )
            return
    ...
    def get_predicate_info(self, name: str, arity: int) -> list[PredicateInfo] | None:
        """述語情報を取得"""
        key = predicate_key(name, arity)
    ...
    def get_statistics(self) -> dict[str, int]:
        ...
        total_references = sum(
            len(p.references) for plist in self.predicates.values() for p in plist
        )
        return {
