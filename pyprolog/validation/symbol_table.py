...
class SymbolTable:
    ...
    def _init_builtins(self):
        """ビルトイン述語を初期化"""
        builtin_predicates = ['var/1', 'atom/1', 'number/1', 'functor/3', 'arg/3', '=../2', 'asserta/1', 'assertz/1', 'retract/1', 'member/2', 'append/3', 'findall/3', 'get_char/1', 'write/1', 'nl/0', 'is/2', '=/2', '=:=/2', '=\\=/2', '</2', '=</2', '>/2', '>=/2', 'true/0', 'fail/0', '!/0', 'length/2', 'reverse/2', 'sort/2', 'bagof/3', 'setof/3', 'call/1', 'once/1', 'repeat/0', 'halt/0', 'halt/1', 'read/1', 'peek_char/1', 'at_end_of_stream/0', 'read_line/1']
        for predicate in builtin_predicates:
            self.builtins.add(predicate)

    def add_predicate(self, name: str, arity: int, rule_or_fact: Union[Rule, Fact], file_path: Optional[str]=None, line_number: int=0) -> None:
        """述語定義を追加"""
        key = predicate_key(name, arity)
        predicate_info = PredicateInfo(name=name, arity=arity, definition=rule_or_fact, file_path=file_path, line_number=line_number)
    ...
    def add_reference(self, name: str, arity: int, referenced_in: Union[Rule, Fact], file_path: Optional[str]=None, line_number: int=0, context: str='body') -> None:
        """述語参照を追加"""
        ...
        if container_key not in self.predicates:
            logger.warning("Attempted to add reference to a container rule '%r' that is not in the symbol table.", container_key)
            return
        reference = PredicateReference(name=name, arity=arity, referenced_in=referenced_in, file_path=file_path, line_number=line_number, context=context)
    ...
    def get_statistics(self) -> Dict[str, int]:
        ...
        total_references = sum((len(p.references) for plist in self.predicates.values() for p in plist))
        return {'total_predicates': total_predicates, 'total_definitions': total_definitions, 'total_references': total_references, 'builtin_predicates': len(self.builtins), 'user_defined_predicates': len(self.user_defined)}
