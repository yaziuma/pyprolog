    ...
    def validate(
        ...
        try:
            ...
            if check_type == "all" or check_type == "conflicts":
                issues = self.analyzers["conflicts"].analyze(
            ...
            logger.info(
                f"バリデーション完了: {len(all_issues)} 個の問題, {analysis_duration:.2f}秒"
            )
            return result
    ...
    def _analyze_rule_or_fact(
        ...
        if isinstance(rule_or_fact, Fact):
            ...
            self.symbol_table.add_predicate(
                name, arity, rule_or_fact, file_path, line_number
            )
            ...
            for term in body_terms:
                ...
                self.symbol_table.add_reference(
                    ref_name, ref_arity, rule_or_fact, file_path, line_number, "body"
                )
        ...
        for predicate_infos in self.symbol_table.predicates.values():
            for predicate_info in predicate_infos:
                if isinstance(predicate_info.definition, Rule):
                    complexity = self._calculate_rule_complexity(
                        predicate_info.definition
                    )
                    if complexity > 10:  # 閾値
        ...
        def count_disjunctions(term):
            if hasattr(term, "functor"):
                ...
                if functor_name == ";":
                    return (
                        1
                        + count_disjunctions(term.args[0])
                        + count_disjunctions(term.args[1])
                    )
            return 0
