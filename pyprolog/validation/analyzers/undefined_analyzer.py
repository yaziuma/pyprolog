...
class UndefinedAnalyzer:
    ...
    def _check_undefined_predicates(self, symbol_table: SymbolTable) -> List[ValidationIssue]:
        """未定義述語のチェック"""
        issues = []
    ...
    def _check_potential_typos(self, symbol_table: SymbolTable) -> List[ValidationIssue]:
        """スペルミスの可能性をチェック"""
        issues = []
        ...
        for undefined_key in referenced_predicates.keys():
            ...
            if symbol_table.is_defined(name, arity) or symbol_table.is_builtin(name, arity):
                continue
            
            # 似ている定義済み述語を検索
            similar_predicates = self._find_similar_predicates(name, arity, defined_predicates)
    ...
    def _check_missing_imports(self, symbol_table: SymbolTable, dependency_graph: DependencyGraph) -> List[ValidationIssue]:
        """欠落したインポートや依存関係をチェック"""
        issues = []
    ...
    def _find_similar_predicates(self, name: str, arity: int, defined_predicates: Set[str]) -> List[str]:
        """類似の述語を検索"""
        similar = []
