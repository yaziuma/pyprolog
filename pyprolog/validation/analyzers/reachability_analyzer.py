...
class ReachabilityAnalyzer:
    ...
    def _get_entry_points(self, symbol_table: SymbolTable) -> Set[str]:
        ...
        if not entry_points:
            logger.warning("明示的なエントリーポイントが見つからないため、全ての事実をエントリーポイントとします")
    ...
    def _is_entry_point(self, predicate_name: str, predicate_info) -> bool:
        ...
        # 特定の命名規則に従う述語
        entry_patterns = [
            'main_', 'test_', 'query_', 'demo_', 'example_', 
            'run_', 'start_', 'init_', 'setup_'
        ]
        ...
        # 外部から参照される可能性の高い述語
        common_entry_names = [
            'solve', 'find', 'search', 'check', 'verify', 
    ...
    def _should_ignore_unreachable(self, predicate_name: str) -> bool:
        ...
        ignore_patterns = [
            # ヘルパー関数やユーティリティ
            'helper_', 'util_', 'aux_', 'internal_',
            # テスト関連（別途テストから呼ばれる可能性）
            'test_', 'check_', 'verify_',
            # デバッグ用
            'debug_', 'trace_', 'print_',
            # 将来の拡張用
            'future_', 'todo_', 'planned_'
        ]
    ...
    def _check_isolated_components(self, symbol_table: SymbolTable, dependency_graph: DependencyGraph) -> List[ValidationIssue]:
        """孤立したコンポーネントをチェック"""
        issues = []
        ...
        for component in components:
            if len(component) > 1:  # 複数の述語からなる成分
                has_entry_point = any(predicate_key in entry_points for predicate_key in component)
