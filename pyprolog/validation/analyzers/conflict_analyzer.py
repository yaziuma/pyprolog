...
class ConflictAnalyzer:
    ...
    def _check_multiple_definitions(self, symbol_table: SymbolTable) -> List[ValidationIssue]:
        """複数定義のチェック"""
        issues = []
        ...
        for predicate_key, predicate_infos in multiple_defs.items():
            ...
            for i, fact1_info in enumerate(facts):
                for fact2_info in facts[i+1:]:
                    if self._facts_potentially_conflict(fact1_info.definition, fact2_info.definition):
            ...
            if len(rules) > 1:
                ...
                for i, rule1_info in enumerate(rules):
                    for rule2_info in rules[i+1:]:
                        if self._rules_potentially_conflict(rule1_info.definition, rule2_info.definition):
                            issue = ValidationIssue(
                                issue_type="conflict",
                                severity="info",
    ...
    def _check_logical_conflicts(self, symbol_table: SymbolTable) -> List[ValidationIssue]:
        """論理的矛盾のチェック"""
        issues = []
    ...
    def _check_type_inconsistencies(self, symbol_table: SymbolTable) -> List[ValidationIssue]:
        """型の不整合チェック"""
        issues = []
    ...
    def _check_infinite_recursion(self, symbol_table: SymbolTable, dependency_graph: DependencyGraph) -> List[ValidationIssue]:
        """無限再帰の可能性をチェック"""
        issues = []
        ...
        for cycle in cycles:
            if len(cycle) > 1:  # 自己再帰以外の循環
                ...
                for predicate_key in cycle:
                    predicate_infos = symbol_table.get_predicate_info(*predicate_key.split('/'))
                    if predicate_infos:
    ...
    def _same_argument_structure(self, term1: Any, term2: Any) -> bool:
        ...
        try:
            ...
            for arg1, arg2 in zip(args1, args2):
                ...
                if self._is_variable_or_wildcard(str(arg1)) or self._is_variable_or_wildcard(str(arg2)):
                    continue
                return False
    ...
    def _is_variable_or_wildcard(self, arg_str: str) -> bool:
        ...
        return (arg_str.startswith('_') or 
                arg_str in ['X', 'Y', 'Z'] or
                arg_str.isupper())
    
    def _check_conflicting_arguments(self, pred1: PredicateInfo, pred2: PredicateInfo) -> bool:
        """対照的な述語の引数が競合するかチェック"""
        try:
            if isinstance(pred1.definition, Fact) and isinstance(pred2.definition, Fact):
                return self._same_argument_structure(pred1.definition.head, pred2.definition.head)
            return False
        except Exception:
            return False
    ...
    def _has_base_case(self, predicate_info: PredicateInfo, symbol_table: SymbolTable) -> bool:
        """述語にベースケースがあるかチェック"""
