...
class TraceFormatter:
    ...
    def _safe_format_text(events: List[TraceEvent]) -> str:
        ...
        for event in events:
            ...
            if event.event_type == "CALL":
                ...
                rule_info = f" (via {type(event.rule_ref).__name__})" if event.rule_ref else ""
    ...
    def format_json(events: List[TraceEvent], query: str, solutions: List[Dict]) -> str:
        ...
        try:
            ...
            return json.dumps({
                "error": f"Error formatting trace: {e}",
                "event_count": len(events)
            }, indent=2, ensure_ascii=False)
    
    @staticmethod
    def _safe_format_json(events: List[TraceEvent], query: str, solutions: List[Dict]) -> str:
        """安全なJSON形式フォーマット"""
        trace_data = []
        ...
        for event in events:
            if event.event_type == "CALL":
                ...
                if event.depth < len(stack) and stack[event.depth]:
                    ...
                    stack[event.depth]["bindings"] = TraceFormatter._serialize_bindings(event.bindings)
                    if event.rule_ref:
    ...
    def _render_tree(nodes: List[Dict], prefix: str = "") -> List[str]:
        ...
        for i, node in enumerate(nodes):
            ...
            status_symbol = "✓" if node["status"] == "success" else "✗" if node["status"] == "failed" else "●"
            ...
            result.append(f"{prefix}{symbol} {status_symbol} {node['goal']}{bindings_str}")
            
            # 子ノードの描画
            if node["children"]:
                child_prefix = prefix + ("  " if is_last else "│ ")
                result.extend(TraceFormatter._render_tree(node["children"], child_prefix))
