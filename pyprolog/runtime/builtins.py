    ...
    def execute(self, runtime: 'Runtime', env: BindingEnvironment) -> Iterator[BindingEnvironment]:
        ...
        if isinstance(atom_arg, Atom) and (not isinstance(number_arg, Number)):
            ...
            if number_value is not None:
                ...
                unified, final_env = runtime.logic_interpreter.unify(self.args[1], target_number, env)
                if unified:
                    yield final_env
            return
        elif isinstance(number_arg, Number) and (not isinstance(atom_arg, Atom)):
            target_atom = Atom(str(number_arg.value))
            unified, final_env = runtime.logic_interpreter.unify(self.args[0], target_atom, env)
            if unified:
                yield final_env
            return
...
class FunctorPredicate(BuiltinPredicate):
    ...
    def execute(self, runtime: 'Runtime', env: BindingEnvironment) -> Iterator[BindingEnvironment]:
        ...
        if not isinstance(term, Variable) or (isinstance(term, Variable) and (not env.is_unbound(term.name))):
            ...
            if not isinstance(functor_val, (Atom, Number)):
                return
            if not isinstance(arity_val, Number) or not arity_val.value.is_integer() or arity_val.value < 0:
                return
...
class ArgPredicate(BuiltinPredicate):
    ...
    def execute(self, runtime: 'Runtime', env: BindingEnvironment) -> Iterator[BindingEnvironment]:
        ...
        if not isinstance(index_val, Number) or not index_val.value.is_integer() or index_val.value <= 0:
            return
    ...
    def execute(self, runtime: 'Runtime', env: BindingEnvironment) -> Iterator[BindingEnvironment]:
        ...
        if not term_is_var_unbound:
            ...
            while isinstance(current_cell, Term) and current_cell.functor.name == '.' and (len(current_cell.args) == 2):
                py_list.append(runtime.logic_interpreter.dereference(current_cell.args[0], env))
            ...
            if isinstance(functor_from_list, Number) and args_from_list:
                return
            if isinstance(functor_from_list, Atom) and functor_from_list.name == '[]' and args_from_list:
                return
            MAX_ARITY = 50
...
class DynamicAssertAPredicate(BuiltinPredicate):
    ...
    def execute(self, runtime: 'Runtime', env: BindingEnvironment) -> Iterator[BindingEnvironment]:
        ...
        logger.debug('ASSERTA: Dereferenced clause_val: %s (type: %s)', clause_val, type(clause_val))
        if isinstance(clause_val, Variable):
            logger.warning('ASSERTA: Attempt to assert an uninstantiated variable: %s. Failing.', clause_val)
            return
        if not isinstance(clause_val, (Term, Atom)):
            logger.warning('ASSERTA: Attempt to assert a non-term/non-atom: %s (type: %s). Failing.', clause_val, type(clause_val))
            return
        try:
            ...
            if clause_val_as_term.functor.name == ':-' and len(clause_val_as_term.args) == 2:
                ...
                logger.debug('ASSERTA: Identified as rule. Head: %s, Body: %s', head, body)
                if not isinstance(head, (Term, Atom)):
                    logger.warning('ASSERTA: Rule head is not Term or Atom: %s. Failing on clause: %s', head, clause_val)
                    return
                ...
                if isinstance(body, Atom):
                    ...
                    logger.debug('ASSERTA: Converted Atom body %s to Term: %s', body, processed_body)
                elif not isinstance(body, Term):
                    logger.warning('ASSERTA: Rule body %s (type: %s) is not an Atom or Term. Failing assertion for clause: %s', body, type(body), clause_val)
                    return
            ...
            logger.debug('ASSERTA: About to yield environment for: %r', clause_val_as_term)
            yield env
...
class DynamicAssertZPredicate(BuiltinPredicate):
    ...
    def execute(self, runtime: 'Runtime', env: BindingEnvironment) -> Iterator[BindingEnvironment]:
        ...
        clause_val = runtime.logic_interpreter.dereference(self.args[0], env)
        logger.debug('ASSERTZ: Dereferenced clause_val: %r (type: %r)', clause_val, type(clause_val))
        if isinstance(clause_val, Variable):
            logger.warning('ASSERTZ: Attempt to assert an uninstantiated variable: %r. Failing.', clause_val)
            return
        if not isinstance(clause_val, (Term, Atom)):
            logger.warning('ASSERTZ: Attempt to assert a non-term/non-atom: %r (type: %r). Failing.', clause_val, type(clause_val))
            return
        try:
            clause_val_as_term = Term(clause_val, []) if isinstance(clause_val, Atom) else clause_val
            ...
            if clause_val_as_term.functor.name == ':-' and len(clause_val_as_term.args) == 2:
                head = clause_val_as_term.args[0]
                body = clause_val_as_term.args[1]
                logger.debug('ASSERTZ: Identified as rule. Head: %s, Body: %s', head, body)
                if not isinstance(head, (Term, Atom)):
                    logger.warning('ASSERTZ: Rule head is not Term or Atom: %s. Failing on clause: %s', head, clause_val)
                    return
                if isinstance(head, Atom):
                    head = Term(head, [])
                ...
                if isinstance(body, Atom):
                    processed_body = Term(body, [])
                    logger.debug('ASSERTZ: Converted Atom body %s to Term: %s', body, processed_body)
                elif not isinstance(body, Term):
                    logger.warning('ASSERTZ: Rule body %s (type: %s) is not an Atom or Term. Failing assertion for clause: %s', body, type(body), clause_val)
                    return
                new_rule = Rule(head, processed_body)
            ...
            logger.debug('ASSERTZ: About to yield environment for: %s', clause_val_as_term)
            yield env
            logger.debug('ASSERTZ: Successfully yielded environment for: %s', clause_val_as_term)
        except Exception as e:
            logger.error('ASSERTZ: Unexpected Python exception during assertion of %s: %s', clause_val, e, exc_info=True)
            return
...
class MemberPredicate(BuiltinPredicate):
    ...
    def execute(self, runtime: 'Runtime', env: BindingEnvironment) -> Iterator[BindingEnvironment]:
        ...
        while isinstance(current_list, Term) and isinstance(current_list.functor, Atom) and (current_list.functor.name == '.') and (len(current_list.args) == 2):
            head = current_list.args[0]
            head = current_list.args[0]
            tail = current_list.args[1]
            logger.debug("MEMBER: Loop iteration. current_list='%r', head='%r', tail='%r', element_to_match='%r'", current_list, head, tail, element_to_match)
            unified, next_env = runtime.logic_interpreter.unify(element_to_match, head, env)
            logger.debug("MEMBER: Unify result for element '%r' and head '%r': %r. Env after unify: %r", element_to_match, head, unified, next_env.bindings if unified else 'N/A')
            if unified:
                yield next_env
            current_list_before_deref = tail
            current_list = runtime.logic_interpreter.dereference(tail, env)
            logger.debug("MEMBER: Tail dereferenced. Before='%r', After='%r', Type After='%r'", current_list_before_deref, current_list, type(current_list))
        return
...
class AppendPredicate(BuiltinPredicate):
    ...
    def execute(self, runtime: 'Runtime', env: BindingEnvironment) -> Iterator[BindingEnvironment]:
        ...
        unified_l1_empty, env_clause1_after_l1 = runtime.logic_interpreter.unify(self.args[0], Atom('[]'), env_clause1)
        logger.debug("APPEND_CP1: L1='%r', L2='%r', L3='%r'. Trying to unify L1 with [].", self.args[0], self.args[1], self.args[2])
        if unified_l1_empty:
            logger.debug('APPEND_CP1: L1 unified with []. Env after L1 unify: %r', env_clause1_after_l1.bindings)
            unified_l2_l3, final_env_clause1 = runtime.logic_interpreter.unify(self.args[1], self.args[2], env_clause1_after_l1)
            if unified_l2_l3:
                logger.debug('APPEND_CP1: L2 and L3 unified. Yielding solution from CP1. Env: %r', final_env_clause1.bindings)
                yield final_env_clause1
            else:
                logger.debug('APPEND_CP1: L2 and L3 failed to unify.')
        else:
            logger.debug('APPEND_CP1: L1 failed to unify with [].')
        logger.debug("APPEND_CP2: L1='%r', L2='%r', L3='%r'. Creating patterns.", self.args[0], self.args[1], self.args[2])
        ...
        logger.debug("APPEND_CP2: Attempting to unify L1 ('%r') with pattern '%r'.", self.args[0], list1_pattern)
        unified_l1_cons, env_clause2_after_l1 = runtime.logic_interpreter.unify(self.args[0], list1_pattern, env_clause2)
        if unified_l1_cons:
            logger.debug("APPEND_CP2: L1 unified with '%r'. Env after L1 unify: %r", list1_pattern, env_clause2_after_l1.bindings)
            list3_pattern = Term(Atom('.'), [h1_var, t3_var])
            logger.debug("APPEND_CP2: Attempting to unify L3 ('%r') with pattern '%r'.", self.args[2], list3_pattern)
            unified_l3_cons, env_clause2_after_l3 = runtime.logic_interpreter.unify(self.args[2], list3_pattern, env_clause2_after_l1)
            if unified_l3_cons:
                logger.debug("APPEND_CP2: L3 unified with '%r'. Env after L3 unify: %r", list3_pattern, env_clause2_after_l3.bindings)
                logger.debug('APPEND_CP2: Making recursive call: append(%r, %r, %r) with env: %r', t1_var, self.args[1], t3_var, env_clause2_after_l3.bindings)
...
class DynamicRetractPredicate(BuiltinPredicate):
    ...
    def execute(self, runtime: 'Runtime', env: BindingEnvironment) -> Iterator[BindingEnvironment]:
        ...
        logger.debug('RETRACT: Dereferenced clause_to_retract_for_unify: %r (type: %r)', clause_to_retract_for_unify, type(clause_to_retract_for_unify))
        if isinstance(clause_to_retract_for_unify, Variable):
            logger.warning('RETRACT: Attempt to retract an uninstantiated variable: %r. Failing (Instantiation Error).', clause_to_retract_for_unify)
            return
        if not isinstance(clause_to_retract_for_unify, (Term, Atom)):
            logger.warning('RETRACT: Argument is not a Term or Atom: %r (type: %r). Failing (Type Error).', clause_to_retract_for_unify, type(clause_to_retract_for_unify))
            return
        ...
        is_retracting_rule_form = isinstance(target_clause_struct, Term) and target_clause_struct.functor.name == ':-' and (len(target_clause_struct.args) == 2)
        target_head_to_match = target_clause_struct.args[0] if is_retracting_rule_form else target_clause_struct
        target_body_to_match = target_clause_struct.args[1] if is_retracting_rule_form else None
        for i in range(len(runtime.rules) - 1, -1, -1):
            db_clause = runtime.rules[i]
            renamed_db_clause = runtime.logic_interpreter._rename_variables(db_clause, env)
            db_head: Term
            ...
            if unified_head:
                if is_retracting_rule_form:
                    ...
                    if not isinstance(db_body_term, Term) and target_body_to_match is not None:
                        continue
                    unified_body, final_env = runtime.logic_interpreter.unify(target_body_to_match, db_body_term, head_env)
                    if unified_body:
                        logger.info('RETRACT: Matched and removed rule: %r', runtime.rules[i])
                        runtime.logic_interpreter.remove_rule(db_clause)
                        yield final_env
                        return
                else:
                    logger.info('RETRACT: Matched and removed clause: %r (using head match for fact-form retract)', runtime.rules[i])
                    runtime.logic_interpreter.remove_rule(db_clause)
                    yield head_env
                    return
...
class AtEndOfStreamPredicate(BuiltinPredicate):
    ...
    def _get_target_stream(self, runtime: 'Runtime', env: BindingEnvironment):
        ...
        if len(self.args) == 0:
            ...
            if unified_input and isinstance(unified_input.input_handler, StreamInputHandler):
...
class ListingPredicate(BuiltinPredicate):
    ...
    def execute(self, runtime: 'Runtime', env: BindingEnvironment) -> Iterator[BindingEnvironment]:
        ...
        try:
            ...
            formatter = PrologFormatter(variable_mapper=getattr(runtime, 'variable_mapper', None), functor_mapper=getattr(runtime, 'functor_mapper', None))
    ...
    def _parse_predicate_spec(self, spec: PrologType, env: BindingEnvironment, runtime: 'Runtime') -> tuple:
        ...
        try:
            ...
            if isinstance(dereferenced_spec, Term):
                if isinstance(dereferenced_spec.functor, Atom) and dereferenced_spec.functor.name == '/' and (len(dereferenced_spec.args) == 2):
...
class ExportFactsPredicate(BuiltinPredicate):
    ...
    def _get_functor_name(self, functor: PrologType, runtime: 'Runtime') -> str:
        ...
        if isinstance(functor, Atom):
            ...
            return str(functor)
from .io_predicates import GetCharPredicate as UnifiedGetCharPredicate, ReadLinePredicate as UnifiedReadLinePredicate, PeekCharPredicate as UnifiedPeekCharPredicate
