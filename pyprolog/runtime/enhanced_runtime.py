...
class EnhancedRuntime(Runtime):
    ...
    def _trace_call(self, goal, env, call_id):
        ...
        print(
            f"{indent}     ENV: {dict(list(env.bindings.items())[:3])}{'...' if len(env.bindings) > 3 else ''}"
        )
    ...
    def _trace_exit(self, goal, result, call_id, solution_num):
        ...
        if result and result.bindings:
            print(
                f"{indent}     RESULT: {dict(list(result.bindings.items())[:2])}{'...' if len(result.bindings) > 2 else ''}"
            )
